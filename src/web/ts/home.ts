...
(() => {
  ...
  const normalizeNotes = (payload: unknown): LgxpkfNote[] => {
    if (Array.isArray(payload)) return payload as LgxpkfNote[];
    if (payload && typeof payload === "object") {
      const data = payload as { items?: LgxpkfNote[]; notes?: LgxpkfNote[] };
  ...
  const loadList = async (path: string, list: HTMLElement | null, status: HTMLElement | null, emptyText: string): Promise<void> => {
    ...
    try {
      const payload = await apiJson<unknown>(path, state.token);
      const items = normalizeNotes(payload);
      list.innerHTML = "";
      if (!items.length) {
